...
def create_theme(db: Session, theme: schemas.Theme):
    db_theme = models.Theme(**theme.model_dump(), problems=[])
    db.add(db_theme)
    db.commit()
    db.refresh(db_theme)
    return db_theme
...
class ProblemDAO:
    ...
    def find_problems_by_chapter_and_difficulty(self, db: Session, chapter_id: int, difficulty: int) -> List[models.Problem]:
