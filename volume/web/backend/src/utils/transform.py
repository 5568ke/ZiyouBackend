...
from ..crud import create_program, create_problem, create_theme, create_keyword, create_chapter, create_edition, create_edition_chapter, get_theme_by_name,create_keyword, get_keyword_by_text
from ..schemas import ProgramCreate, ProblemCreate, ThemeCreate, Keyword, ChapterCreate, EditionCreate, EditionChapterCreate,ProblemCreate,ProgramCreate,KeywordCreate
...
def transform_quiz(db: Session):
    ...
    # 開啟SQL檔案
    with open("/usr/backend/datafiles/quizbase_20241004.sql", 'r', encoding='utf-8') as file:
        # 讀取檔案內容
    ...
    for insert_statement in insert_patterns:
        ...
        for i,quiz_str in enumerate(quiz_list):
            ...
            for theme_name in themes_list[:-1]:
                ...
                if not theme:
                    theme_create = ThemeCreate(theme=theme_name)  
                    theme = create_theme(db, theme_create) 
                problem.themes.append(theme)

            keywords_list = quiz_data[6].split(' ')
            for keyword_text in keywords_list:
                keyword = get_keyword_by_text(db, keyword_text) 
                if not keyword:
                    keyword_create = KeywordCreate(keyword=keyword_text)
                    keyword = create_keyword(db, keyword_create)
                add_keyword_to_problem(db, problem.id, keyword)
...
def determine_chapter_id(db: Session, ziyou_chapter_dict, quiz_data):
    ...
    if chapter:
        ...
        print("can't find chapter with chapter name : ",chapter_name)
        return -1
...
def determine_parameter(quiz_data):
    ...
    file_path = "/usr/backend/datafiles/datfile/datfile/" + program_name + ".dat" 
    try:
...
def transform_edition_chapter_data(db: Session) -> None:
    ...
    for item in edition_comparison_list:
        ...
        print("other : ",other_version_chapters)
...
def transform_theme(db: Session) -> None:
    ...
    for theme_entry in theme_list:
        theme = ThemeCreate(theme=theme_entry)
        create_theme(db, theme)

        

def add_keyword_to_problem(db: Session, problem_id: int, keyword: Keyword):
    association_exists = db.query(models.problem_keyword_table).filter_by(problem_id=problem_id, keyword_id=keyword.id).first()

    if not association_exists:
        stmt = models.problem_keyword_table.insert().values(problem_id=problem_id, keyword_id=keyword.id)
        db.execute(stmt)
        db.commit()
